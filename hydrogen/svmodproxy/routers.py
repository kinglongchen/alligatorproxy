...
class Public():
	def add_routes(self,mapper):
		svmanp_controller=controller=wsgi.Resource(controllers.VMSVManProxy())
...
# 		mapper.connect('/svs/{id}',controller=svmanp_controller,action='svCall')
		...
		mapper.connect('/svs/{id}/{sv_inst_id}',controller=svmanp_controller,action='svSuspend',conditions=dict(method=["PUT"]))
